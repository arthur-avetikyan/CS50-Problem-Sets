...
function Brick:hit(inPlayBrickCount)
    ...
    if self.tier > 0 then
        ...
        if self.color == 1 then
            self.inPlay = false    
            inPlayBrickCount = inPlayBrickCount - 1    
        else
            self.color = self.color - 1
        end        
    end
...
function Brick:render()
    if self.inPlay then
        if self.key then
            love.graphics.draw(gTextures['main'], gFrames['bricks'][#gFrames['bricks']], self.x, self.y)
        else
