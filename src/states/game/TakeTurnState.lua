...
function TakeTurnState:enter(params)
    self:attack(self.firstPokemon, self.secondPokemon, self.firstSprite,
                self.secondSprite, self.firstBar, self.secondBar, function()
        ...
        self:attack(self.secondPokemon, self.firstPokemon, self.secondSprite,
                    self.firstSprite, self.secondBar, self.firstBar, function()
...
function TakeTurnState:attack(attacker, defender, attackerSprite,
                              defenderSprite, attackerkBar, defenderBar, onEnd)
    ...
    gStateStack:push(BattleMessageState(attacker.name .. ' attacks ' ..
                                            defender.name .. '!',
                                        function() end, false))
    ...
    Timer.after(0.5, function()
        ...
        Timer.every(0.1, function()
            ...
            Timer.every(0.1, function()
                defenderSprite.opacity =
                    defenderSprite.opacity == 64 and 255 or 64
            end):limit(6):finish(function()
...
function TakeTurnState:faint()
    ...
    Timer.tween(0.2, {[self.playerSprite] = {y = VIRTUAL_HEIGHT}}):finish(
        function()
...
function TakeTurnState:victory()
    ...
    Timer.tween(0.2, {[self.opponentSprite] = {y = VIRTUAL_HEIGHT}}):finish(
        function()
            -- play victory music
            gSounds['battle-music']:stop()
            ...
            gStateStack:push(BattleMessageState('Victory!', function()
                ...
                local exp = (self.opponentPokemon.HPIV +
                ...
                gStateStack:push(BattleMessageState(
                                     'You earned ' .. tostring(exp) ..
                                         ' experience points!', function() end,
                                     false))
                ...
                Timer.after(1.5, function()
                    ...
                    Timer.tween(0.5, {
                        [self.battleState.playerExpBar] = {
                            value = math.min(
                                self.playerPokemon.currentExp + exp,
                        ...
                        self.playerPokemon.currentExp =
                            self.playerPokemon.currentExp + exp

                        -- level up if we've gone over the needed amount
                        if self.playerPokemon.currentExp >
                            self.playerPokemon.expToLevel then
                            ...
                            self.playerPokemon.currentExp =
                                self.playerPokemon.currentExp -
                                    self.playerPokemon.expToLevel
                            local HPIncrease, attackIncrease, defenseIncrease,
                                  speedIncrease = self.playerPokemon:levelUp()

                            gStateStack:push(
                                BattleMessageState('Congratulations! Level Up!',
                                                   function() end), false)

                            gStateStack:push(
                                BattleStatIncreaceState(self.battleState, self.playerPokemon,
                                                        HPIncrease, attackIncrease,
                                                        defenseIncrease, speedIncrease,
...
function TakeTurnState:fadeOutWhite()
    -- fade in
    gStateStack:push(FadeInState({r = 255, g = 255, b = 255}, 1, function()
