...
function Level:init()
    ...
    table.insert(self.aliens, Alien(self.world, 'square', VIRTUAL_WIDTH - 80,
                                    VIRTUAL_HEIGHT - TILE_SIZE - ALIEN_SIZE / 2,
                                    'Alien'))

    -- spawn a few obstacles
    table.insert(self.obstacles,
                 Obstacle(self.world, 'vertical', VIRTUAL_WIDTH - 120,
                          VIRTUAL_HEIGHT - 35 - 110 / 2))
    table.insert(self.obstacles,
                 Obstacle(self.world, 'vertical', VIRTUAL_WIDTH - 35,
                          VIRTUAL_HEIGHT - 35 - 110 / 2))
    table.insert(self.obstacles,
                 Obstacle(self.world, 'horizontal', VIRTUAL_WIDTH - 80,
                          VIRTUAL_HEIGHT - 35 - 110 - 35 / 2))

    -- ground data
    self.groundBody = love.physics.newBody(self.world, -VIRTUAL_WIDTH,
                                           VIRTUAL_HEIGHT - 35, 'static')
    self.groundFixture = love.physics
                             .newFixture(self.groundBody, self.edgeShape)
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        for k, alien in pairs(self.launchMarker.aliens) do
            ...
            if love.keyboard.wasPressed('space') and not self.hasHit and
                not self.hasSplit then
                self.hasSplit = true

                local alienOne = Alien(self.world, 'round', xPos,
                                       math.min(yPos, VIRTUAL_HEIGHT - 35),
                                       'Player')
                alienOne.body:setLinearVelocity(xVel, yVel + 50)
                alienOne.fixture:setRestitution(0.4)
                alienOne.body:setAngularDamping(1)

                local alienTwo = Alien(self.world, 'round', xPos,
                                       math.min(yPos, VIRTUAL_HEIGHT - 35),
                                       'Player')
                alienTwo.body:setLinearVelocity(xVel, yVel - 50)
                alienTwo.fixture:setRestitution(0.4)
                alienTwo.body:setAngularDamping(1)

                table.insert(self.launchMarker.aliens, alienOne)
                table.insert(self.launchMarker.aliens, alienTwo)
        ...
        for k, alien in pairs(self.launchMarker.aliens) do
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) or xPos >
                VIRTUAL_WIDTH + ALIEN_SIZE or xPos < -ALIEN_SIZE then
                stopped = stopped + 1
            end
        end
        ...
        if stopped >= #self.launchMarker.aliens then
            ...
            if #self.aliens == 0 then gStateMachine:change('start') end
        end
    end
end
...
function Level:render()
    ...
    for x = -VIRTUAL_WIDTH, VIRTUAL_WIDTH * 2, 35 do
        love.graphics.draw(gTextures['tiles'], gFrames['tiles'][12], x,
                           VIRTUAL_HEIGHT - 35)
    end

    self.launchMarker:render()

    for k, alien in pairs(self.aliens) do alien:render() end

    for k, obstacle in pairs(self.obstacles) do obstacle:render() end

    -- render instruction text if we haven't launched bird
    if not self.launchMarker.launched then
        love.graphics.setFont(gFonts['medium'])
        love.graphics.setColor(0, 0, 0, 255)
        love.graphics.printf('Click and drag circular alien to shoot!', 0, 64,
    ...
    if #self.aliens == 0 then
        ...
        love.graphics.setColor(0, 0, 0, 255)
        love.graphics.printf('VICTORY', 0, VIRTUAL_HEIGHT / 2 - 32,
                             VIRTUAL_WIDTH, 'center')
        love.graphics.setColor(255, 255, 255, 255)
    end
end
